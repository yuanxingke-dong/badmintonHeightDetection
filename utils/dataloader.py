...
class CenternetDataset(Dataset):
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        cx, cy = (left + right) / 2, (top + bottom) / 2  # 羽毛球的中心点

        # 下面做裁切出来图片的左上角点在原图中的位置(x,y)
        w_ = w * 4 / 5
        h_ = h * 4 / 5
        if not cx < w_ / 2 and not iw - cx < w_ / 2:
            x = cx - w_ / 2
        elif cx < w_ / 2:
            x = 0
        else:
        ...
        if not cy < h_ / 2 and not ih - cy < h_ / 2:
            y = cy - h_ / 2
        elif cy < h_ / 2:
            y = 0
        else:
